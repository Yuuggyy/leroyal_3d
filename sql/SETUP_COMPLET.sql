...
-- ────────────────────────────────────────────────────────────
-- ÉTAPE 9 : MENU LE ROYAL (burgers, fruits de mer, fast-food)
INSERT INTO public.categories (nom, description, emoji, ordre, actif) VALUES
('Burgers',               'Burgers maison',                '🍔', 1, true),
('Sandwichs & Wraps',     'Sandwichs et wraps',             '🥪', 2, true),
('Frittes & Snacks',      'Accompagnements et snacks',      '🍟', 3, true),
('Fruits de Mer',         'Crevettes, poissons, calamars',  '🦐', 4, true),
('Desserts',              'Douceurs et glaces',             '🍰', 5, true),
('Boissons',              'Boissons fraîches et chaudes',   '🥤', 6, true)
ON CONFLICT DO NOTHING;

INSERT INTO public.produits (nom, description, prix, categorie_id, disponible, ordre) VALUES
('Classic Burger',           'Pain brioché, steak boeuf, salade, tomate, sauce maison',  5.00,  (SELECT id FROM categories WHERE nom='Burgers'), true, 1),
('Royal Beef Burger',        'Double steak, cheddar, bacon, sauce royale',               7.00,  (SELECT id FROM categories WHERE nom='Burgers'), true, 2),
('Cheese Burger',            'Steak boeuf, cheddar fondu, cornichons',                   6.00,  (SELECT id FROM categories WHERE nom='Burgers'), true, 3),
('Chicken Burger',           'Filet de poulet croustillant, salade, mayo',              5.50,  (SELECT id FROM categories WHERE nom='Burgers'), true, 4),
('Spicy Burger',             'Steak epice, jalapeños, sauce piquante',                  6.50,  (SELECT id FROM categories WHERE nom='Burgers'), true, 5),
('Sandwich Club',            'Poulet, bacon, oeuf, tomate, laitue',                      7.00,  (SELECT id FROM categories WHERE nom='Sandwichs & Wraps'), true, 1),
('Wrap Poulet',              'Filet de poulet, legumes croquants, sauce',               6.00,  (SELECT id FROM categories WHERE nom='Sandwichs & Wraps'), true, 2),
('Sandwich Thon',            'Thon, mayonnaise, salade, tomate',                        6.00,  (SELECT id FROM categories WHERE nom='Sandwichs & Wraps'), true, 3),
('Frittes Classiques',       'Pommes de terre dorees',                                  2.50,  (SELECT id FROM categories WHERE nom='Frittes & Snacks'), true, 1),
('Frittes Speciales',        'Frittes assaisonnees epices maison',                      3.50,  (SELECT id FROM categories WHERE nom='Frittes & Snacks'), true, 2),
('Nuggets x6',               '6 pieces de nuggets dores',                               4.50,  (SELECT id FROM categories WHERE nom='Frittes & Snacks'), true, 3),
('Onion Rings',              'Rondelles d oignon panees',                               3.50,  (SELECT id FROM categories WHERE nom='Frittes & Snacks'), true, 4),
('Poisson Frit',             'Tilapia ou capitaine frit, citron, piment',               8.00,  (SELECT id FROM categories WHERE nom='Fruits de Mer'), true, 1),
('Crevettes Grillees',       'Crevettes marinees, grillees au feu',                    12.00,  (SELECT id FROM categories WHERE nom='Fruits de Mer'), true, 2),
('Calamars Frits',           'Calamars panes, sauce tartare',                           10.00,  (SELECT id FROM categories WHERE nom='Fruits de Mer'), true, 3),
('Plateau Fruits de Mer',    'Crevettes + poisson + calamars pour 2',                  22.00,  (SELECT id FROM categories WHERE nom='Fruits de Mer'), true, 4),
('Glace 2 boules',           'Vanille, chocolat ou fraise',                             3.50,  (SELECT id FROM categories WHERE nom='Desserts'), true, 1),
('Brownie Chocolat',         'Fondant au chocolat, noix',                               4.00,  (SELECT id FROM categories WHERE nom='Desserts'), true, 2),
('Coca Cola 33cl',           NULL,                                                       2.00,  (SELECT id FROM categories WHERE nom='Boissons'), true, 1),
('Fanta 33cl',               NULL,                                                       2.00,  (SELECT id FROM categories WHERE nom='Boissons'), true, 2),
('Jus Naturel',              'Mangue, passion ou ananas',                               2.50,  (SELECT id FROM categories WHERE nom='Boissons'), true, 3),
('Eau Minerale 75cl',        NULL,                                                       1.50,  (SELECT id FROM categories WHERE nom='Boissons'), true, 4),
('Milkshake',                'Vanille, chocolat ou fraise',                             4.00,  (SELECT id FROM categories WHERE nom='Boissons'), true, 5)
ON CONFLICT DO NOTHING;
